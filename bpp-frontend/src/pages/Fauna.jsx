...
import { getDetailImageUrl, getThumbnailImageUrl } from "../utils/cloudinaryImages";
...
const Fauna = () => {
  ...
  const handleDelete = async () => {
    ...
    } catch (error) { setError(`Error al eliminar: ${error.message}`); }
            ...
            .map((animal) => (
              <StyledCard key={animal.id} onClick={() => handleOpenDetail(animal)}>
                <CardMedia
                  component="img"
                  height="140"
                  image={getThumbnailImageUrl(animal.image)}
                  alt={animal.name}
                  loading="lazy"
                  decoding="async"
                />
        ...
        {viewingAnimal && (
          <>
             ...
             <DialogContent>
               <CardMedia
                 component="img"
                 image={getDetailImageUrl(viewingAnimal.image)}
                 alt={viewingAnimal.name}
                 sx={{ borderRadius: 2, mb: 2, maxHeight: 400, objectFit: 'contain' }}
               />
